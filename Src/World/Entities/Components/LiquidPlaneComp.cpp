...
void LiquidPlaneComp::MaybeUpdate(const World& world)
{
	...
	const glm::ivec3 deltas[] = { glm::ivec3(1, 0, 0),  glm::ivec3(-1, 0, 0), glm::ivec3(0, 1, 0),
...
void LiquidPlaneComp::GenerateMesh()
{
	...
	for (const glm::ivec3& waterCoord : m_underwater)
	{
		...
		for (int dx = 0; dx < 2; dx++)
		{
			for (int dz = 0; dz < 2; dz++)
			{
				...
				if (it == indexMap.end())
				{
					...
					vertex.edgeDists[1] =
						static_cast<int>(!dz && !IsUnderwater(waterCoord - glm::ivec3(0, 0, 1))) * 255;
					vertex.edgeDists[2] = static_cast<int>(dx && !IsUnderwater(waterCoord + glm::ivec3(1, 0, 0))) * 255;
					vertex.edgeDists[3] =
						static_cast<int>(!dx && !IsUnderwater(waterCoord - glm::ivec3(1, 0, 0))) * 255;
				}
		...
		const glm::ivec2 faceIndexOffsets[] = { { 0, 0 }, { 0, 1 }, { 1, 0 }, { 0, 1 }, { 1, 1 }, { 1, 0 } };
