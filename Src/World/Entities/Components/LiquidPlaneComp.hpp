...
public:
	...
	const std::vector<glm::ivec3> UnderwaterCells() const { return m_underwater; }

	bool OutOfDate() const { return m_outOfDate; }

	void MarkOutOfDate() { m_outOfDate = true; }

	uint32_t NumIndices() const { return m_numIndices; }

	const eg::Buffer& VertexBuffer() const { return m_vertexBuffer; }

	const eg::Buffer& IndexBuffer() const { return m_indexBuffer; }
