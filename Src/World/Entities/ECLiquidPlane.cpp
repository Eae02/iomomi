...
void ECLiquidPlane::HandleMessage(eg::Entity& entity, const EditorSpawnedMessage& message)
{
	m_shouldGenerateMesh = true;
	m_outOfDate = true;
}

void ECLiquidPlane::HandleMessage(eg::Entity& entity, const EditorWallsChangedMessage& message)
{
	m_shouldGenerateMesh = true;
	m_outOfDate = true;
}

void ECLiquidPlane::HandleMessage(eg::Entity& entity, const EditorMovedMessage& message)
{
	m_shouldGenerateMesh = true;
	m_outOfDate = true;
}
...
void ECLiquidPlane::MaybeUpdate(const eg::Entity& entity, const World& world)
{
	...
	glm::vec3 entityPos = eg::GetEntityPosition(entity);
	glm::vec3 startF = entityPos + glm::vec3(DirectionVector(entity.GetComponent<ECWallMounted>().wallUp)) * 0.5f;
...
void ECLiquidPlane::GenerateMesh(const eg::Entity& entity)
{
	const glm::vec3 entityPos = eg::GetEntityPosition(entity);
	...
	for (const glm::ivec3& waterCoord : m_underwater)
	{
		...
		float y = std::min<float>(entityPos.y, waterCoord.y + 1);
...
eg::MeshBatch::Mesh ECLiquidPlane::GetMesh() const
{
