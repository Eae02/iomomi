...
struct QualityVariable
{
	...
	QualityVariable(T vLow, T low, T medium, T high, T vHigh) noexcept : values{ vLow, low, medium, high, vHigh } {}

	template <class = std::enable_if<std::is_same_v<T, bool>>>
	QualityVariable(QualityLevel minLevel) noexcept
		: values{ minLevel <= QualityLevel::VeryLow, minLevel <= QualityLevel::Low, minLevel <= QualityLevel::Medium,
		          minLevel <= QualityLevel::High, minLevel <= QualityLevel::VeryHigh }
	{
	}
};
