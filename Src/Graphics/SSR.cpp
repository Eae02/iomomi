...
void SSR::CreatePipeline()
{
	...
	pipeline1CI.fragmentShader.specConstantsData = &SSR_SAMPLES[(int)settings.reflectionsQuality];
	pipeline1CI.fragmentShader.specConstantsDataSize = sizeof(SSR_SAMPLES[0]);
	m_pipeline = eg::Pipeline::Create(pipeline1CI);
	m_pipeline.FramebufferFormatHint(GetFormatForRenderTexture(RenderTex::SSRTemp1));
	...
	pipelineBlurCI.fragmentShader.specConstantsData = &SSR_BLUR_RADII[(int)settings.reflectionsQuality];
...
float* ssrBlurFalloff = eg::TweakVarFloat("ssr_blur_fo", 7.5f, 0.0f);
...
void SSR::Render(eg::TextureRef waterDepth, RenderTex destinationTexture, RenderTexManager& rtManager, const eg::ColorSRGB& fallbackColor)
{
	...
	rpBeginInfo.framebuffer = rtManager.GetFramebuffer(RenderTex::SSRTemp1, {}, {}, "SSR");
	...
	eg::DC.PushConstants(0, sizeof(float) * 3, &fallbackColorLin.r);
	...
	eg::DC.EndRenderPass();
	rtManager.RenderTextureUsageHintFS(RenderTex::SSRTemp1);
	...
	const float pc1[4] = { *ssrBlurIntensity, 0, *ssrBlurFalloff, *ssrBlurMax };
	...
	const float pc2[4] = { 0, *ssrBlurIntensity * (float)rtManager.ResY() / (float)rtManager.ResX(), *ssrBlurFalloff, *ssrBlurMax };
