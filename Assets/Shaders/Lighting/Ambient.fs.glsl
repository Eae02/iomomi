...
vec3 calcReflection(vec3 surfacePos, vec3 dirToEye, vec3 normal)
{
	...
	for (uint i = 1; i <= ssrLinearSamples; i++)
	{
		...
		if (behindDepthBuffer(sampleWorldPos))
		{
			...
			fade *= 1 - (lo / ssrLinearSamples);
			return mix(ambient, texture(gbColor1Sampler, sampleTC).rgb, fade);
		}
	}
...
vec3 CalculateLighting(GBData gbData)
{
	...
	if ((gbData.flags & RF_NO_SSR) == 0)
	{
		reflection = calcReflection(gbData.worldPos, toEye, gbData.normal);
	}
	
	return (gbData.albedo * kd * ambient + reflection * fresnel * (1 - gbData.roughness)) * gbData.ao;
}
